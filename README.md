...
1. Уставленные python и doxygen. 
...
3. Должны быть описаны массивы для входных и выходных данных. Их будут использовать тестируемые функции (названия массивов должны совпадать с названиями, указанными в сценарии производительности).
...
При использовании этого ключа __необходимо__ передать скрипту дополнительные ключи:
...
При использовании этого ключа передать скрипту дополнительные ключи __не нужно__.
...
При использовании этого ключа __необходимо__ передать скрипту дополнительные ключи (*-i, -b* и *-p*), см. выше описание ключа config.
...
### Ключи для дополнительной настройки скрипта 
...
Сначала нужно запустить install.bat.Этот скрипт создаст переменную окружения с путем к testperf.

#### Запуск с ключом config

python %testperf%/testperf.py config -i \<path> -b \<path>

#### Запуск с ключом run

python %testperf%/testperf.py run

#### Запуск с ключом all

python %testperf%/testperf.py all -i \<path> -b \<path>

#### Запуск с ключом outdir

python %testperf%/testperf.py outdir
